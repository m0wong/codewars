...
function dirReduc(arr){
  const obj = {'NORTH': [0, 1], 'SOUTH': [0, -1], 'WEST': [-1, 0], 'EAST': [1, 0]};
  const r1 = arr.map(s=>s=obj[s]).reduce((x,y)=>[x[0]+y[0],x[1]+y[1]]);
  let r2 = Array(Math.abs(r1[0]) + Math.abs(r1[1]));
  if(r1[0] > 0){
    r2.fill('EAST',0, Math.abs(r1[0]));
  }else if(r1[0] < 0){
    r2.fill('WEST',0, Math.abs(r1[0]));
  }
  if(r1[1] > 0){
    r2.fill('NORTH', Math.abs(r1[0]), Math.abs(r1[0]) + Math.abs(r1[1]));
  }else if(r1[1] < 0){
    r2.fill('SOUTH', Math.abs(r1[0]), Math.abs(r1[0]) + Math.abs(r1[1]));
  }
  return r2;
}
​
